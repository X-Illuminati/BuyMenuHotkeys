...
local function BMHK_Check_Keybinds(self, key, down)
    ...
    for _, currentButton in ipairs(self.upgradeButtons) do
        if currentButton.TechId == kTechId.Crush then
        ...
        elseif currentButton.TechId == kTechId.Vampirism then
        ...
        elseif currentButton.TechId == kTechId.Silence then
            silencebutton = currentButton
    ...
    if GetIsBinding(key, kBMHKFocusKey) then
        if not down then
            ...
            elseif aurabutton and aurabutton.Selected then
                ...
                focusbutton.Purchased = false
                table.removevalue(self.upgradeList, kTechId.Foucs)
                AlienBuy_OnUpgradeDeselected()
            end // if focusbutton...
        end // if not down
        retval = true
        ...
        if not down then
            ...
            if adrenbutton and not adrenbutton.Selected and AlienBuy_GetTechAvailable(kTechId.Adrenaline) then
                if combat_mode_not_active then
                    ...
                    if silencebutton and silencebutton.Selected then
                        silencebutton.Selected = false
                        silencebutton.Purchased = false
                        table.removevalue(self.upgradeList, kTechId.Silence)
                        AlienBuy_OnUpgradeDeselected()
                    end
                end
    ...
    if GetIsBinding(key, kBMHKSilenceKey) then
        if not down then
            //Print("--Toggle Silence")
            if silencebutton and not silencebutton.Selected and AlienBuy_GetTechAvailable(kTechId.Silence) then
                if combat_mode_not_active then
                    if adrenbutton and adrenbutton.Selected then
                        adrenbutton.Selected = false
                        adrenbutton.Purchased = false
                ...
                silencebutton.Selected = true
                silencebutton.Purchased = false
                table.insertunique(self.upgradeList, kTechId.Silence)
                AlienBuy_OnUpgradeSelected()
            elseif silencebutton and silencebutton.Selected then
                silencebutton.Selected = false
                silencebutton.Purchased = false
                table.removevalue(self.upgradeList, kTechId.Silence)
                AlienBuy_OnUpgradeDeselected()
            end // if silencebutton...
        end // if not down
        retval = true
    end // if key == bmhk_silence
...
local function BMHK_Create_Button_Label(TechId)
    ...
    elseif (TechId == kTechId.Vampirism) and ("None" ~= BindingsUI_GetInputValue(kBMHKVampirismKey)) then
        ...
        hotkey_graphic:SetAnchor(GUIItem.Middle, GUIItem.Bottom)
    ...
    elseif (TechId == kTechId.Celerity) and ("None" ~= BindingsUI_GetInputValue(kBMHKCelerityKey)) then
        ...
        hotkey_graphic:SetAnchor(GUIItem.Left, GUIItem.Center)
    elseif (TechId == kTechId.Silence) and ("None" ~= BindingsUI_GetInputValue(kBMHKSilenceKey)) then
        hotkey_graphic, hotkey_text = GUICreateButtonIcon(kBMHKSilenceKey, true)
        hotkey_graphic:SetAnchor(GUIItem.Left, GUIItem.Center)
...
local function BMHK_Set_Label_Position(label, TechId, xcorr, ycorr)
    ...
    if label and (label ~= 1) then
        ...
        elseif TechId == kTechId.Celerity then
            newpos = Vector(-size.x+xcorr,0,0)
        elseif TechId == kTechId.Silence then
            newpos = Vector(-size.x+xcorr,0,0)
...
// - Add buttom labeles for Silence, Vampirism, Crush, and Focus
